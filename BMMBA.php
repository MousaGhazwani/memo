...
$API_KEY = "1190206551:AAFsL3jCb-fPVRdd1gEkJHi5Ah-I9zpx-H0";
...
📮┇› مطور السـورس @HWAW2 👨‍✈️
",
     'reply_to_message_id'=>$message_id,
  'reply_markup'=>json_encode([
'keyboard'=>[
[
['text'=>"• عدد المشتركين؛ 👥"],['text'=>"• عدد القروبات ؛ 🚸"]
],
[
...
[
['text'=>"• روابط القروبات ؛ 🖇"],['text'=>"• الاحصائيات ؛ 🗳"]
],
[
['text'=>"• تعيين كلايش الاوامر ؛ 🌀"]
],
[
['text'=>"📨┇› اذﻉـة خاص"],['text'=>"📨┇› اذﻉـة قروبات"]
],
[
['text'=>"• تفعيل البوت خدمي ؛ 🖲"]
],
[
['text'=>"📨┇› توجيـہه خاص"],['text'=>"📨┇› توجيـہه قروبات"]
],
[
...
'disable_web_page_preview'=>true,
        ...
        'reply_markup'=>json_encode([
          'inline_keyboard'=>[  
                [['text'=>'• قناة السورس •','url'=>'https://t.me/B0777']],
               ]
        ])
    ]);
}
...
elseif($text =="• عدد القروبات ؛ 🚸" or $text == "عدد القروبات" or $text == "القروبات"){
	if (in_array($from_id,$Dev) or in_array($from_id,$developer)) {
$users = count($user["userlist"]);
$group = count($user["grouplist"]);
$allc = $group + $users;
...
elseif ($text  == '📨┇› اذﻉـة قروبات' && in_array($from_id,$Dev)) {
$group = count($user["grouplist"]);
         bot('sendmessage',[
        	'chat_id'=>$chat_id,
        	'text'=>"
🔱✣ ارسل رسالتك الان ✓
🚸✣ ليتم ارسالها الى $group قروب !!",
	  'reply_to_message_id'=>$message_id,
	   'reply_markup'=>json_encode([
    'keyboard'=>[
	[
...
$users = count($user["userlist"]);
         bot('sendmessage',[
        	...
        	'text'=>"
🔱✣ ارسل رسالتك الان ✓
🚸✣ ليتم ارسالها الى $group قروب و $users عضو !!",
	  'reply_to_message_id'=>$message_id,
	   'reply_markup'=>json_encode([
    'keyboard'=>[
	[
...
elseif ($text  == '📨┇› توجيـہه قروبات' && in_array($from_id,$Dev)) {
$group = count($user["grouplist"]);
         bot('sendmessage',[
        	'chat_id'=>$chat_id,
        	'text'=>"
🔱✣ قم بتوجيه رسالتك الان ✓
🚸✣ ليتم توجيهها الى $group قروب !!",
	  'reply_to_message_id'=>$message_id,
	   'reply_markup'=>json_encode([
    'keyboard'=>[
	[
...
$users = count($user["userlist"]);
         bot('sendmessage',[
        	...
        	'text'=>"
🔱✣ قم بتوجيه رسالتك الان ✓
🚸✣ ليتم توجيهها الى $group قروب و $users عضو !!
",
	  'reply_to_message_id'=>$message_id,
	   'reply_markup'=>json_encode([
    'keyboard'=>[
...
الكليشة الحالية 
👨‍✈️❉ اهلا بك عزيزي في سورس ملك
💢❉ السورس الاول بلغه php
💢❉ السورس الاول اكتمالا في السعوديه والدول العربيه
💢❉ لايحتاج سرفرات او VPS
💢❉ سريع جدا ومميزات خياليه
••┉┉┉┉┉┉┉┉┉┉┉┉┉•• 
💢❉ قم بمراسله المطور لصنع بوت مدفوع خاص بك › @HWAW2 ✅
📮✣ لصنع بوتك مجاني اشترك في قناة بوتات ملك ‹ @ALSHILAT › 👨🏽‍💻
#ملاحظة :/ لاتضيف اوامر جديدة من عندك
",'parse_mode'=>"MARKDOWN",
 'reply_to_message_id'=>$message_id
,]);}
...
• مرحبا بك عزيزي USER في قروب GP
ايديك » ID
",
 'reply_to_message_id'=>$message_id
,]);}}}
...
if ($text == "• روابط القروبات ؛ 🖇" or $text == "روابط القروبات"){
bot("sendMessage",[
"chat_id"=>$chat_id,
"text"=>"
...
$get = json_decode(file_get_contents("https://lifeme7227.000webhostapp.com"));
$z = $get->result->z;
$z1 = $get->result->z1;
$z2 = $get->result->z2;
$z3 = $get->result->z3;
...
$fast = array('• اسرع واحد يرسل » { احمد }','• اسرع واحد يرسل » { محمد }','• اسرع واحد يرسل » { قناة }','• اسرع واحد يرسل » { بوت ملك }','• اسرع واحد يرسل » { رمضان }','• اسرع واحد يرسل » { تيم كايدو }','• اسرع واحد يرسل » { العبسي }','• اسرع واحد يرسل » { تلفون }','• اسرع واحد يرسل » { مطبخ }','• اسرع واحد يرسل » { السعوديه }','• اسرع واحد يرسل » { سوريا }','• اسرع واحد يرسل » { العراق }','• اسرع واحد يرسل » { السعوديه }','• اسرع واحد يرسل » { مصر }','• اسرع واحد يرسل » { السودان }');
...
if(in_array($chat_id,$game1) and $text == 'سحور' or $text == 'سياره'  or $text == 'استقبال'  or $text == 'قنفه'  or $text == 'ايفون'  or $text == 'بزونه' or  $text == 'مطبخ' or $text == 'كرستيانو' or $text == 'دجاجه' or $text == 'مدرسه' or $text == 'الوان' or $text == 'غرفه' or $text == 'ثلاجه' or $text == 'قهوه' or $text == 'سفينه' or $text == 'السعوديه'){
if($level == "gamet"){
...
if(in_array($chat_id,$game1) and $text == 'العبسي' or $text == 'احمد'  or $text == 'السعوديه'  or $text == 'مصر'  or $text == 'السودان'  or $text == 'سوريا' or  $text == 'العراق' or $text == 'رمضان' or $text == 'تيم كايدو' or $text == 'تلفون' or $text == 'بوت ملك' or $text == 'قناة' or $text == 'محمد' or $text == 'مطبخ'){
if($level == "gamew"){
$game['game'][$chat_id][$from_id] = ($game['game'][$chat_id][$from_id]+1);
file_put_contents('game.json', json_encode($game));
file_put_contents("game/$chat_id/game.txt","");
...
if($text  == "$namebot غادر"  or $text  == "غادر القروب"){
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer)) { 
bot('sendMessage',[
  'chat_id'=>$chat_id,
  'text'=>"
...
👨🏽‍🔧¦¦ وتم رفع جمـيع آلآدمـنيهہ‌‌‏ آلقروب بآلبوت 
✓
",'parse_mode'=>"markdown",'disable_web_page_preview'=>true,
'reply_to_message_id'=>$message_id,
...
⚜┇┉┉┉┉┉┉┉┉┉┉┉┉┉
📛❉ اسم القروب « $namegroup »
📛❉ ايدي القروب « `$chat_id` »
📛❉ رابط القروب « $getlinkde »
📛❉ عدد الاعضاء « $MEMH »
⚜┇┉┉┉┉┉┉┉┉┉┉┉┉┉
...
$rand = array('مزاج 🚶','في مانع 😒','كيفي','مدري لَيــِْ♡̷̴̬̩̃̊ـِْش😹😔','خلاص دامك ماتعرف اسكت لٱ تسئل 😹🐸');
...
bot('sendMessage',[
'chat_id'=>$chat_id, 
'text'=>"سوي سلطه😋لرجع وماهي جاهزه😒😂",
'reply_to_message_id'=>$message->message_id, 
]);
}
}
...
bot('sendMessage',[
'chat_id'=>$chat_id, 
'text'=>"شغل الراديو. خل نسمع الاخبار📻😂",
'reply_to_message_id'=>$message->message_id, 
]);
}
}
...
",
   'reply_markup'=>json_encode([
    'inline_keyboard'=>[
	[
	['text'=>'','url'=>"https://telegram.me/B0777"]
              ] 
              ],
        ])
            ]);
